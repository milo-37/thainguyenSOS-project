...
// MapView (đã nhận onSelect/onOpenMedia/onTransfer/onClaim)
    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
            ...
            <circle
                ...
                strokeLinecap="round"
            />
            <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" fontSize="14" fontWeight={800}>
                {Math.round(v * 100)}%
            </text>
        </svg>
    );
...
export default function AdminDashboard() {
    ...
    // Khi vào tab "Chưa phân công" mặc định lọc trạng thái = tiếp nhận
    useEffect(() => {
    ...
    // mở detail sheet khi đã có selectedId
    useEffect(() => {
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            <div className="mx-auto max-w-7xl p-4 md:p-6 space-y-4">
                ...
                {stats && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div className="rounded-xl border bg-white shadow-sm px-4 py-3 flex items-center justify-between gap-3">
                            ...
                            <Button variant="outline" size="sm" className="rounded-lg h-9 px-3 shrink-0" onClick={() => setVatTuOpen(true)}>
                ...
                {/* ===== Bottom Sheet: Chi tiết yêu cầu (map full width) ===== */}
                <Sheet
                ...
                >
                    <SheetContent side="bottom" className="rounded-t-2xl max-h-[85vh] overflow-auto">
                        <SheetHeader>
                            <SheetTitle>
                                Chi tiết yêu cầu #{selectedRow?.id ?? ''}
                            </SheetTitle>
                        </SheetHeader>
                        ...
                        {selectedRow ? (
                            <div className="mt-3 space-y-4">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    <Badge
                                        variant="outline"
                                        className={`rounded-full ${statusClass(((selectedRow.trang_thai as TrangThaiCode) ?? 'tiep_nhan') as TrangThaiCode)}`}
                                    >
                                        {STATUS_MAP[((selectedRow.trang_thai as TrangThaiCode) ?? 'tiep_nhan') as TrangThaiCode]?.label ?? 'Tiếp nhận'}
                                    </Badge>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                ...
                                                setStatusValue(((selectedRow.trang_thai as TrangThaiCode) ?? 'dang_xu_ly') as TrangThaiCode);
                                                setStatusNote('');
                                            }}
                                        >
                                            Cập nhật trạng thái
                ...
                >
                    <DialogContent className="sm:max-w-[520px] rounded-2xl">
                        ...
                        <DialogFooter>
                            <Button variant="outline" className="rounded-xl" onClick={() => setOpenTransfer({ open: false, row: null })}>
                                Hủy
                            </Button>
                ...
                >
                    <DialogContent className="sm:max-w-[520px] rounded-2xl">
                        ...
                        <DialogFooter>
                            <Button variant="outline" className="rounded-xl" onClick={() => setOpenStatus({ open: false, row: null })}>
                                Hủy
                            </Button>
                            <Button
                                className="rounded-xl"
                                ...
                                <div key={h.id ?? idx} className="py-3 text-sm">
                                    <div className="flex items-center gap-2">
                                        <Badge variant="outline" className="rounded-full">
                                            {h.trang_thai_hien_thi ?? h.trang_thai ?? ''}
                                        </Badge>
                                        <div className="text-xs text-muted-foreground">
                    ...
                    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center">
                        ...
                        <div className="max-w-5xl w-full px-6">
                            {(() => {
                                ...
                                if (!m) return null;
                                return m.type === 'image' ? (
                                    <img src={m.url} alt="" className="w-full max-h-[80vh] object-contain rounded-2xl" />
                                ) : (
                                    <video src={m.url} className="w-full max-h-[80vh] rounded-2xl" controls autoPlay />
                                );
                            })()}
                        </div>
                    </div>
