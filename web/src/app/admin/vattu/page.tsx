...
export default function VatTuPage(){
    ...
    const openCreate = ()=>{ setEditing(null); setFrm({ten:'',donvi:'',ma:'',ghichu:''}); setOpen(true); };
    const openEdit = (r:VatTu)=>{ setEditing(r); setFrm({ten:r.ten, donvi:r.donvi||'', ma:r.ma||'', ghichu:r.ghichu||''}); setOpen(true); };

    const save = async()=>{
        if (!frm.ten.trim()) return alert('Nhập tên vật tư');
        try{
            setSaving(true);
            if (editing) await updateVatTu(editing.id, frm);
            else await createVatTu(frm);
            setOpen(false);
            await load(editing ? meta.current_page : 1);
            alert(editing ? 'Đã cập nhật' : 'Đã thêm');
        }catch(err:any){ alert(err?.message||'Lỗi lưu'); }
        finally{ setSaving(false); }
    };

    const remove = async(id:number)=>{
        if (!confirm('Xóa vật tư này?')) return;
        try{ await deleteVatTu(id); await load(meta.current_page); }catch(e:any){ alert(e?.message||'Xóa thất bại'); }
    };
                    ...
                    <div className="bg-white rounded-lg w-full max-w-lg p-4 space-y-3">
                        ...
                        <div className="grid gap-3">
                            <div>
                                ...
                                <input className="mt-1 h-10 border rounded px-3 w-full" value={frm.ten} onChange={e=>setFrm({...frm,ten:e.target.value})}/>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-sm">Mã</label>
                                    <input className="mt-1 h-10 border rounded px-3 w-full" value={frm.ma||''} onChange={e=>setFrm({...frm,ma:e.target.value})}/>
                                </div>
                                <div>
                                    <label className="text-sm">Đơn vị tính</label>
                                    <input placeholder="Cái/chai/thùng" className="mt-1 h-10 border rounded px-3 w-full" value={frm.donvi||''} onChange={e=>setFrm({...frm,donvi:e.target.value})}/>
                                </div>
                            </div>
                            <div>
