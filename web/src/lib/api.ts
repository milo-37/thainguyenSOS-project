...
export async function api(path: string, init: RequestInit = {}) {
  ...
  // Nếu body là string => thường là JSON => set Content-Type nếu chưa có
...
// Warehouse APIs (nếu backend bạn có)
...
export async function listYeuCauAdmin(params: any = {}, init: RequestInit = {}) {
  const usp = new URLSearchParams(params).toString();
...
export async function quickUpdateTrangThai(
  id: number,
  trang_thai: "tiep_nhan" | "dang_xu_ly" | "hoan_thanh" | "huy",
  ghi_chu: string
) {
...
export async function createRole(body: { name: string }) {
  return api(`/admin/roles`, { method: "POST", body: JSON.stringify(body) });
}
export async function updateRole(id: number, body: { name: string }) {
  return api(`/admin/roles/${id}`, { method: "PUT", body: JSON.stringify(body) });
}
export async function deleteRole(id: number) {
